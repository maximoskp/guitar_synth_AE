...
def get_adsr_from_audio( s, N = 600 ):
    ab = np.abs( s )
    v_max = np.max( ab )
    av = np.convolve(ab, np.linspace(1,0,N)/N, mode='same')
    av = (av/np.max(av))*v_max
    i_max = np.argmax( av )
    i_min = np.where( av[i_max:] < 0.01*v_max )[0][0]
    curve_adsr = np.hstack( ( np.linspace(0,v_max, i_max) , np.linspace(v_max,0, i_min), np.zeros( ab.size - i_max - i_min ) ) )
    return curve_adsr

# how to run
# c = get_adsr_from_audio( s )
# p = make_sawtooth_with_adsr( freq = 200, amp=0.5, adsr = c, sr=16000 )
# get freq from string-fret-freq table
# amp should be taken as 1/total_notes, since all amps are 1
